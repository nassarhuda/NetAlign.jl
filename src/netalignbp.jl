...
----------
    ...
    For details: [https://www.cs.purdue.edu/homes/dgleich/publications/bayati%202009%20-%20network%20alignment.pdf]
    Usage
    ----
    Input:
    ----
    ...
    - `a`: the value of alpha in the netalignbp objective (weight of the matching)
    - `b`: the value of beta in the netalignbp objective (weight of the overlap)
    ...
    - `dtype`: the rounding type (default 1)
    - `maxiter`: maximum number of iterations to take (default 1000)
    - `verbose`: output at each iterations (default false)

    Methods:
    -------
    xbest,st,status,hist = netalignbp(S,w,a,b,li,lj,gam,dtype,maxiter,verbose)
    xbest,st,status,hist = netalignbp(S,w,a,b,li,lj,gam,dtype,maxiter)
    xbest,st,status,hist = netalignbp(S,w,a,b,li,lj,gam,dtype)
    xbest,st,status,hist = netalignbp(S,w,a,b,li,lj,gam)
